...
import { GeoNamesCity, Params } from 'toco-lib';

@Component({
  selector: 'org-view-geo-names-city',
...
export class OrgViewGeoNamesCityComponent {
  ...
  public ngOnInit(): void {
    if (this.value) {
      if (this.value.geonames_admin1)
        this._createValueAsArray(
          this.value.geonames_admin1,
          this._geonames_admin1
        );
      if (this.value.geonames_admin2)
        this._createValueAsArray(
          this.value.geonames_admin2,
          this._geonames_admin2
        );
  ...
  private _createValueAsArray(
    objFromCopy: Params<any>,
    arrayToFill: Params<any>[]
  ): void {
